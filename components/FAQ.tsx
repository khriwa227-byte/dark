...
const FAQItem: React.FC<{ question: string; answer: string }> = ({ question, answer }) => {
  ...
  return (
    <div className={`py-8 px-6 transition-all duration-300 rounded-3xl ${isOpen ? 'glass-card mb-4 mt-4 shadow-xl' : ''}`}
      style={{ borderBottom: isOpen ? 'none' : '1px solid rgba(5,150,105,0.15)' }}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex justify-between items-center text-left"
      >
        <span className="text-2xl font-black tracking-tight pr-8 transition-colors" style={{ color: '#111827' }}>{question}</span>
        <div className={`w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all duration-300 flex-shrink-0 ${isOpen ? 'rotate-180' : ''}`}
          style={isOpen
            ...
            : { backgroundColor: 'transparent', borderColor: 'rgba(5,150,105,0.4)', color: '#059669' }
          }>
    ...
    <section id="faq" className="py-32 px-6">
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20">
        <div className="text-center lg:text-left">
          <h2 className="text-5xl lg:text-6xl font-black tracking-tighter lg:sticky top-40 leading-tight" style={{ color: '#111827' }}>
            Veelgestelde <br/><span className="text-italics underline decoration-4 underline-offset-8" style={{ textDecorationColor: '#F59E0B' }}>vragen</span>
          </h2>
        </div>
