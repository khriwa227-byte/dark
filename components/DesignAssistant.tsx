...
export const DesignAssistant: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      (window as any).gtag?.('event', 'conversion', { send_to: 'AW-18195439252/cyDuCPTW4rkcEJS9oeRD' });
    ...
    <div className="fixed bottom-8 right-8 z-[60]">
      {isOpen ? (
        ...
        >
          ...
          <span className="absolute -top-1 -right-1 w-4 h-4 border-2 border-white rounded-full" style={{ backgroundColor: '#F59E0B' }}></span>
        </button>
      )}
    </div>
  );
